...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([x1,y1,x2,y2,track_id,track.detections[-1].confidence], dtype=np.float))
        if len(outputs) > 0:
            outputs = np.stack(outputs,axis=0)  # (#obj, 5) (x1,y1,x2,y2,ID,confidence)
        return outputs
